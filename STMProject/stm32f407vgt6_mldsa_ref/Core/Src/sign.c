...
//    if(rej_cnt==trigger_rej_idx){
//  	  for(int i=0;i<N;++i){
//  		  HAL_UART_Transmit(&huart2,(unsigned char*)&cp.coeffs[i],4,1000);
//  	  }
//    }
...
int PQCLEAN_MLDSA44_CLEAN_crypto_sign_signature(uint8_t *sig,
        ...
        const uint8_t *sk,int trigger_rej_idx,
		  int poly_idx, int* rej_num) {
    return PQCLEAN_MLDSA44_CLEAN_crypto_sign_signature_ctx(sig, siglen, m, mlen, NULL, 0, sk,trigger_rej_idx,poly_idx,rej_num);
}
